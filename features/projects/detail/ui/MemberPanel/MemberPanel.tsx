...
import { useProjectMembers } from "@/features/projects/detail/hooks/useProjectMembers";
import { useDeleteProjectMember } from "@/features/projects/detail/hooks/useDeleteProjectMember";
...
export default function MemberPanel({
  ...
  const { data } = useProjectMembers(projectId, { enabled: isLoggedIn });
  const { mutate: kickMember, isPending } = useDeleteProjectMember(projectId);
  const openAlertModal = useOpenAlertModal();

  function handleKick(userId: number, nickname: string) {
    openAlertModal({
      title: `${nickname}님을 내보내시겠습니까?`,
      description: "내보낸 멤버는 프로젝트에서 제외됩니다.",
      positive: {
        text: "내보내기",
