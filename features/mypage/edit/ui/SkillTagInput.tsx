...
import { TECH_STACK } from "@/shared/types/techStack";
...
export default function SkillTagInput({ value, onChange }: Props) {
  ...
  const handleSelect = (key: TechStackKey) => {
    onChange([...value, key]);
    setQuery("");
    setOpen(false);
  };

  const handleRemove = (key: TechStackKey) => {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div ref={containerRef} className="relative">
        <div className="border-border-default flex h-12 w-full items-center justify-between rounded-xl border bg-gray-800 px-3">
          <input
            type="text"
            value={query}
            onChange={(e) => {
      ...
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {value.map((key) => {
            const { label, icon } = TECH_STACK[key];
            return (
              <span
                key={key}
                className="flex items-center gap-1.5 rounded-sm bg-gray-700 px-2 py-1 text-sm text-gray-200"
              >
