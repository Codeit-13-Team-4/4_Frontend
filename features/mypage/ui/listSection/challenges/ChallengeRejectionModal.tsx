...
import { CHALLENGE_REJECTION_MESSAGE } from "@/features/mypage/model/mypage.constants";
import { useRejectChallengeApplication } from "@/features/mypage/hooks/useRejectChallengeApplication";
import {
  ChallengeApplication,
  ReasonCategoryType,
} from "@/features/mypage/model/mypage.types";

interface ChallengeRejectionModalProps {
  challengeId: number;
...
export default function ChallengeRejectionModal({
  challengeId,
  application,
  open,
  onOpenChange,
}: ChallengeRejectionModalProps) {
  const [selectedCategory, setSelectedCategory] =
    useState<ReasonCategoryType | null>(null);
  const [reasonDetail, setReasonDetail] = useState("");
  const [dropdownOpen, setDropdownOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  const { mutate: reject, isPending } =
    useRejectChallengeApplication(challengeId);
  ...
  const handleSelectCategory = (value: ReasonCategoryType, label: string) => {
    setSelectedCategory(value);
    if (value !== "OTHER") {
      setReasonDetail(label);
    } else {
      setReasonDetail("");
    }
    setDropdownOpen(false);
  ...
  const handleSubmit = () => {
    if (!selectedCategory || !reasonDetail.trim()) return;
  ...
  const selectedLabel =
    CHALLENGE_REJECTION_MESSAGE.find((m) => m.value === selectedCategory)
      ?.label ?? null;
      ...
      <Modal.Content className="gap-0 p-10">
          ...
          <div ref={containerRef} className="relative">
            ...
            {dropdownOpen && (
              <ul className="absolute top-full left-0 z-10 mt-1 w-full overflow-hidden rounded-xl border border-gray-700 bg-gray-900 p-1 shadow-lg">
                {CHALLENGE_REJECTION_MESSAGE.map((item) => (
                  <li key={item.value}>
                    <button
                      type="button"
                      ...
                      onClick={() =>
                        handleSelectCategory(
                          item.value as ReasonCategoryType,
                          item.label,
                        )
                      }
        ...
        <Modal.Footer className="mt-6">
          ...
          <Button
            ...
            disabled={!selectedCategory || !reasonDetail.trim() || isPending}
          >
            거절하기
          </Button>
        </Modal.Footer>
