...
export default function MyTabBar({ activeTab, onTabChange }: MyTabBarProps) {
  return (
    <div className="flex justify-center border-b-2 border-gray-700 md:justify-start">
      {MY_TAB_ITEMS.map((tab) => (
        <button
          key={tab.value}
          onClick={() => onTabChange(tab.value)}
          className={`border-b px-6 py-2 text-sm font-semibold text-gray-600 hover:cursor-pointer md:px-10 md:py-4.25 md:text-lg ${
            activeTab === tab.value
